...
class UserShopManager extends Service {
    ...
    */
     ...
     *
     * @return App\Models\Shop\UserShop|bool
     */
    public function buyStock($data, $user) {
        ...
        try {
            ...
            if ($shop->user->id == Auth::user()->id) {
                throw new \Exception("You can't buy from a shop that you own!");
            }
            if (!$shop) {
            ...
            if ($shopStock->cost > 0 && !(new CurrencyManager)->debitCurrency($user, null, 'User Shop Purchase', 'Purchased '.$shopStock->item->name.' from '.$shop->name, $shopStock->currency, $total_cost)) {
            ...
            if (!(new CurrencyManager)->creditCurrency(null, $shop->user, 'User Shop Credit', 'Sold a '.$shopStock->item->displayName.' in '.$shop->displayName, $shopStock->currency, $total_cost)) {
