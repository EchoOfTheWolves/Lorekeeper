<div class="card">
    <div class="card-body">
        ...
        @if (isset($staffView) && $staffView)
            ...
            @if(isset($count['all']) && $count['all'])
                <p>The user has completed this prompt <strong>{{ $count['all'] }}</strong> time{{ $count['all'] == 1 ? '' : 's' }} overall.</p>
                @if ($prompt->limit)
                    <p>They have now submitted this prompt {{ $prompt->limit_period ? $count[$prompt->limit_period] : $count['all'] }} out of {{ $limit }} times {{ $prompt->limit_period ? 'for this '.strtolower($prompt->limit_period) : '' }}.</p>
                @endif
            @endif
        ...
        @else
            ...
            @if(isset($count['all']) && $count['all'])
                ...
                @if($prompt->limit)
                    <p>You have already submitted this prompt {{ $prompt->limit_period ? $count[$prompt->limit_period] : $count['all'] }} out of {{ $limit }} times {{ $prompt->limit_period ? 'for this '.strtolower($prompt->limit_period) : '' }}.</p>
                @endif
            @endif
            <div class="{{ $prompt->limit ? 'text-danger' : '' }}">
