    ...
    <div class="card-body">
        ...
        @if (isset($staffView) && $staffView)
            ...
            @if (isset($count['all']) && $count['all'])
                ...
                @if ($prompt->limit)
                    <p>They have now submitted this prompt {{ $prompt->limit_period ? $count[$prompt->limit_period] : $count['all'] }} out of {{ $limit }} times {{ $prompt->limit_period ? 'for this ' . strtolower($prompt->limit_period) : '' }}.
                    </p>
                @endif
            @endif
            ...
            @if (isset($count['all']) && $count['all'])
                ...
                @if ($prompt->limit)
                    <p>You have already submitted this prompt {{ $prompt->limit_period ? $count[$prompt->limit_period] : $count['all'] }} out of {{ $limit }} times
                        {{ $prompt->limit_period ? 'for this ' . strtolower($prompt->limit_period) : '' }}.</p>
                @endif
            @endif
            <div class="{{ $prompt->limit ? 'text-danger' : '' }}">
